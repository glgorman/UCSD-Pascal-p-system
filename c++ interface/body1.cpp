...
void BODYPART::MAIN(SETOFSYS FSYS, CTP FPROCP)
{	
// locals
...
#if 0
void BODYPART::SELECTOR(SETOFSYS FSYS, CTP FCP)
{
	ATTR	LCP;
	ATTR	LATTR;
	int LMIN, LMAX;
				...
				do {
					...
					if (LATTR.TYPTR!=NULL)
						...
						if (LATTR.TYPTR!=NULL)
							// with LATTR.TYPTR^
						{
						if (COMPTYPES(LATTR.TYPTR.INXTYPE,GATTR.TYPTR))
						{
							if ((INXTYPE!=NULL)&&
						...
						if (GATTR.TYPTR!=NULL)
							if (AISPACKD)
								...
								{
									GATTR.ACCESS=PACKD;
									GEN2((OPRANGE)64/*IXP*/,ELSPERWD,ELWIDTH);
								}
							else
							{
...
void BODYPART::FACTOR(SETOFSYS &FSYS) 
{
	CTP	LCP;
//	CSP	LVP;
...
void BODYPART::TERM(SETOFSYS &FSYS)
{ /*TERM*/
	ATTR		LATTR;
//	STP			LSP;
	OPERATOR	LOP;
...
void BODYPART::EXPRESSION(SETOFSYS FSYS)
{
	/*EXPRESSION*/
...
void BODYPART::NEWSTMT(SETOFSYS &FSYS)
{
//	LABEL 1;
	STP			LSP,LSP1;
	int			VARTS;
...
void BODYPART::STRGVAR(SETOFSYS &FSYS, bool MUSTBEVAR)
{
	EXPRESSION(FSYS);
	// with GATTR
...
void BODYPART::ROUTINE(SETOFSYS &FSYS, int LKEY)
{ /*ROUTINE*/
	switch (LKEY){
		case 12:
			NEWSTMT(FSYS);
...
void BODYPART::VARIABLE(SETOFSYS &FSYS)
{
	CTP LCP;
	if (SY==SYMBOLS::IDENT)
	{
...
void BODYPART::MOVE(SETOFSYS &FSYS, int LKEY)
{
	VARIABLE(FSYS+SYMBOLS::COMMA);
	LOADADDRESS();
	if (SY==SYMBOLS::COMMA)
...
void BODYPART::UNITIO(SETOFSYS &FSYS, int LKEY)
{
	if (GATTR.TYPTR!=INTPTR)
		CERROR(125);
	if (SY==SYMBOLS::COMMA)
...
void BODYPART::CONCAT(SETOFSYS &FSYS)
{
	ADDRRANGE	LLC; 
	int		TEMPLGTH;
...
void BODYPART::COPYDELETE(SETOFSYS &FSYS, int LKEY)
{
	ADDRRANGE	LLC;
	STP			LSP;
...
void BODYPART::STR(SETOFSYS &FSYS)
{
	// with GATTR
...
void BODYPART::CLOSE(SETOFSYS &FSYS)
{
	VARIABLE(FSYS+SYMBOLS::COMMA+SYMBOLS::RPARENT);
	LOADADDRESS();
	if (GATTR.TYPTR!=NULL)
...
void BODYPART::GETPUTETC(SETOFSYS &FSYS, int LKEY)
{
	VARIABLE(FSYS+SYMBOLS::COMMA+SYMBOLS::RPARENT);
	LOADADDRESS();
	if (GATTR.TYPTR!=NULL)
...
void BODYPART::SCAN(SETOFSYS &FSYS)
{
	if (GATTR.TYPTR!=NULL)
		if (GATTR.TYPTR!=INTPTR)
			CERROR(125);
...
void BODYPART::BLOCKIO(SETOFSYS &FSYS, int LKEY)
{
	VARIABLE(FSYS+SYMBOLS::COMMA);
	LOADADDRESS();
	if (GATTR.TYPTR!=NULL)
...
void BODYPART::READ(SETOFSYS &FSYS, int LKEY)
{
	CTP	FILEPTR,LCP;
	bool WASLPARENT;
...
void BODYPART::WRITE(SETOFSYS &FSYS, int LKEY)
{
	STP		LSP;
	bool	DEFAULT;
	CTP		FILEPTR,LCP;
...
void BODYPART::CALLNONSPECIAL(SETOFSYS &FSYS, CTP FCP)
{
//	LABEL 1;
	CTP NXT;
	STP LSP;
...
void BODYPART::COMPOUNDSTATEMENT(SETOFSYS &FSYS)
{
	do	{
		do
			STATEMENT(FSYS+SYMBOLS::SEMICOLON+SYMBOLS::ENDSY);
...
void BODYPART::IFSTATEMENT(SETOFSYS &FSYS)
{
	LBP		LCIX1,LCIX2;
	int		LIC;
...
void BODYPART::CASESTATEMENT(SETOFSYS &FSYS)
{ 
	struct CASEINFO;
...
void BODYPART::REPEATSTATEMENT(SETOFSYS &FSYS)
{ 
	LBP LADDR;
...
void BODYPART::WHILESTATEMENT(SETOFSYS &FSYS)
{
	LBP LADDR, LCIX;
	GENLABEL(&LADDR);
	PUTLABEL(LADDR);
...
void BODYPART::FORSTATEMENT(SETOFSYS &FSYS, CTP &LCP)
{
	ATTR LATTR;
	SYMBOLS::SYMBOL LSY;
	LBP LCIX, LADDR;
...
void BODYPART::WITHSTATEMENT(SETOFSYS &FSYS)
{ 
	CTP LCP;
