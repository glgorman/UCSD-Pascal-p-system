	...
	do {
		...
		if (PUBLICPROCS)
		{
			...
			if (SY!=SYMBOLS::IMPLESY)
			{
				...
				INSYMBOL();
			BLOCK(FSYS - SETOFSYS(4,SYMBOLS::SEPARATSY,SYMBOLS::UNITSY,
				SYMBOLS::INTERSY,SYMBOLS::IMPLESY));
			if (REFBLK>0)
			{
				const unsigned char *data = reinterpret_cast<const unsigned char*>(&REFLIST[0]);
...
// WITH TOS^
	...
	while (SETOFSYS(2,SYMBOLS::UNITSY,SYMBOLS::SEPARATSY).in(SY));
	...
	case FUNC:
		...
		if (NEEDEDBYLINKER)
		{
			...
			while (LCP!=NULL)
			{
				...
				if (SET<128>(2,SEPPREF,SEPFREF).in(LIREC.LITYPE))
				{
					LIREC.FORMAT=BITE;
