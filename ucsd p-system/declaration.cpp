...
//	over, wha tcould possibly go WRONG?!?
...
void DECLARATIONPART::MAIN (SETOFSYS FSYS)
{
	...
	do {
...
#endif
					if (LSY==SYMBOLS::PROGSY)
						if (options.INMODULE)
						{
							CERROR(185 /*SEG DEC !ALLOWED IN UNIT*/);
							PROCDECLARATION(SYMBOLS::PROCSY,false);
						}
						else 
 	...
 	while (!((STATBEGSYS+SYMBOLS::SEPARATSY+
			...
			else
			{
				if (SY==SYMBOLS::IDENT)
				{
					...
					else
					{
						...
						{
							INSYMBOL();
							CONSTANT(FSYS+SYMBOLS::RBRACK,LSP1,LVALU);
							if (LSP1==INTPTR)
							{
				...
				{ 
					...
					LSP = (STRUCTURE*) new STRUCTURE;
					LSP->FORM=SUBRANGE;
					CONSTANT(FSYS+SYMBOLS::COLON,LSP1,LVALU);
					if (STRGTYPE(LSP1))
					{
						CERROR(148);
						LSP1=NULL;
       ...
       // WITH LLEXSTK
		...
		//		NEW(LCP,MODULE);
           // WITH LCP^
				...
				ENTERID(LCP);
				USINGLIST=LCP;
				DECLARATIONPART(this).MAIN(FSYS+SYMBOLS::ENDSY);
...
void DECLARATIONPART::LABELDECLARATION()
{
	bool REDEF;
	LABELP LLP;
	do {
...
void DECLARATIONPART::CONSTDECLARATION()
{
	CTP LCP;
	STP LSP;
	VALU LVALU;
...
//			NEW(LCP,KONST);
			...
			CONSTANT(FSYS+SYMBOLS::SEMICOLON,LSP,LVALU);
			ENTERID(LCP);
			LCP->IDTYPE=LSP;
			LCP->VALUES=LVALU;
			if (SY==SYMBOLS::SEMICOLON)
...
void DECLARATIONPART::TYPEDECLARATION()
{
	CTP LCP,LCP1,LCP2;
	STP LSP;
	ADDRRANGE LSIZE;
...
void DECLARATIONPART::VARDECLARATION()
{
	CTP LCP,NXT,IDLIST;
	STP LSP;
	ADDRRANGE LSIZE;
	...
	do {
		do {
			if (SY==SYMBOLS::IDENT)
			{
				...
				CERROR(2);
			if (!(FSYS+SYMBOLS::COMMA+SYMBOLS::COLON+TYPEDELS).in(SY))
			{
				CERROR(6);
				SKIP(FSYS+SYMBOLS::COMMA+SYMBOLS::COLON+
					SYMBOLS::SEMICOLON+TYPEDELS);
			}
			TEST=SY!=SYMBOLS::COMMA;
			if (!TEST)
				INSYMBOL();
		...
		if (SY==SYMBOLS::COLON)
			...
			CERROR(5);
		IDLIST=NXT;
		TYP((FSYS+SYMBOLS::SEMICOLON+TYPEDELS),LSP,LSIZE);
		while (NXT!=NULL)
		{
			// WITH NXT^
			LCP->IDTYPE=LSP;
	...
	while (!(SY!=SYMBOLS::IDENT)&&!(TYPEDELS.in(SY)));
	if (FWPTR!=NULL)
	{
		CERROR(117);
		FWPTR=NULL;
...
DECLARATIONPART::PROCDECLARATION::PROCDECLARATION(SYMBOLS::SYMBOL FSY, bool SEGDEC)
...
//			NEW(LCP,FUNC,DECLARED,ACTUAL,false);
	...
	if (FSY==SYMBOLS::PROCSY)
	{
		PARAMETERLIST(SET(1,SYMBOLS::SEMICOLON),LCP1,LCP);
		if (!FORW)
			LCP->NEXT=LCP1;
	}
	else
	{
		PARAMETERLIST(SET(2,SYMBOLS::SEMICOLON,SYMBOLS::COLON),LCP1,LCP);
	if (!FORW)
		LCP->NEXT=LCP1;
	if (SY==SYMBOLS::COLON)
	{
...
void DECLARATIONPART::PROCDECLARATION::PARAMETERLIST(SETOFSYS FSY, CTP &FPAR, CTP FCP)
{
	...
	LCP1=NULL; LLC=LC;
	if (!(FSY+SYMBOLS::LPARENT).in(SY))
	{
		CERROR(7);
		SKIP(FSYS+FSY+SYMBOLS::LPARENT);
		if (SY==SYMBOLS::LPARENT)
		{
			if (FORW)
				CERROR(119);
