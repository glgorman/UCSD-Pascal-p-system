...
int TREESEARCH(CTP&,CTP&,ALPHA&);

int TREESEARCH(CTP& n1, CTP& n2, ALPHA &str)
{
	return 0;
}
...
void PASCALCOMPILER::DECLARATIONPART(SETOFSYS FSYS)
{
	ASSERT(0);
}
...
void PASCALCOMPILER::WRITELINKERINFO(bool)
{
	ASSERT(0);
}
...
void PASCALCOMPILER::GETNEXTPAGE()
{
	...
	if (USING)
	{
		if (USEFILE==WORKCODE)
		{
			if (BLOCKREAD(USERINFO.WORKCODE,*SYMBUFP,2,SYMBLK)!=2)
				USING=false;
		}
		else
			if (USEFILE==SYSLIBRARY)
				if (BLOCKREAD(&LIBRARY,*SYMBUFP,2,SYMBLK)!=2)
					USING=false;
		if (!USING)
		{
			SYMBLK=PREVSYMBLK;
	...
	{
		if (INCLUDING)
			if (BLOCKREAD(&INCLFILE,*SYMBUFP,2,SYMBLK)!=2)
			{
				CLOSE((FILE*)(&INCLFILE));
				INCLUDING=false;
				SYMBLK=OLDSYMBLK;
	...
	if (!(INCLUDING||USING))
		if (BLOCKREAD(USERINFO.WORKSYM,*SYMBUFP,2,SYMBLK)!=2)
			CERROR(401);
	if (SYMCURSOR==0)
	{
		if (INMODULE)
...
void PASCALCOMPILER::PRINTLINE()
{
   ...
   if (BPTONLINE)
	   STARORC='*';
   WRITE(0,_LP._tmpfname,SCREENDOTS,6,SEG.val,4,CURPROC.val,5,
...
void PASCALCOMPILER::WRITETEXT()
{
	MOVELEFT(SYMBUFP[SYMCURSOR],CODEP[0],1024);
	if (USERINFO.ERRNUM==0)
       if (BLOCKWRITE(USERINFO.WORKCODE,*CODEP,2,CURBLK)!=2)
         CERROR(402);
     CURBLK=CURBLK+2;
} /*WRITETEXT*/ ;
...
void PASCALCOMPILER::WRITECODE(bool FORCEBUF)
{
	...
	do {
	   ...
	   {
		   if (USERINFO.ERRNUM==0)
			   if (BLOCKWRITE(USERINFO.WORKCODE,*CODEP,CURBLK)!=1)
				CERROR(402);
           CURBLK=CURBLK+1;
		   CURBYTE=0;
	   }
